...
def predict():
    ...
    predictor = joblib.load(open(r"Models/model_xgb.pkl", "rb"))
    scaler = joblib.load(open(r"Models/scaler.pkl", "rb"))
    cv = joblib.load(open(r"Models/countVectorizer.pkl", "rb"))
    try:
        ...
        if "file" in request.files:
            ...
            text_summ = summary(text_input)
            print(text_input)
            return jsonify({"prediction": predicted_sentiment +" " +   text_summ})

    except Exception as e:
        return jsonify({"error": str(e)})
import os 
from openai import OpenAI
x = "123sk-CmEVWnivPn61YtMsPSNyT3BlbkFJ2JB8e6w0QTsiyx2Ozr4Q"
client = OpenAI(api_key=x[3:])
